...
function v3Schema(_context, body) {
...
function v2Schema(_context, schema, contentTypes) {
  if (
    !schema ||
    !contentTypes ||
...
function cdsType(
  ...
  if (schema.$ref) {
    ...
    const resolvedSchema = refType.schema?.$ref
      ? indirectlyReferencedType(context, refType.schema).schema
      : refType.schema;
    if (namedType && normalizeSchemaType(resolvedSchema) === "object") {
      type.kind = "type";
  ...
  switch (schemaType) {
    ...
    case undefined:
      ...
      // last resort — fall through to default (someJSON)
      type = someJSON(context, schema, arrayItem, type);
      break;

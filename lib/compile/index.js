...
function toOpenApiOptions(csdl, csn, options = {}) {
  ...
  const fileOptions = _readConfigFile(callerOptions["config-file"] ?? envOptions["config-file"]);
...
function _readConfigFile(configFilePath) {
  ...
  let fileContent;
  try {
    fileContent = require(configFilePath);
  } catch (err) {
  ...
  for (const key of Object.keys(fileContent)) {
    ...
    result[normalizedKey] = typeof fileContent[key] === "boolean" ? fileContent[key] : JSON.stringify(fileContent[key]);
  }
  return result;
}

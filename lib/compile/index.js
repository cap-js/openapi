...
function toOpenApiOptions(csdl, csn, options = {}) {
  ...
  envOptions.defaultProtocol = envOptions.defaultProtocol ?? "odata";
  ...
  const protocols = _getProtocols(csdl, csn, result.odataVersion, result.defaultProtocol);
...
function _getProtocols(csdl, csn, odataVersion, defaultProtocol) {
  if (csdl.$EntityContainer) {
    ...
    if(odataVersion === "4.01"){
      ...
      protocols.push(defaultProtocol); // matches cds's default protocol when none is annotated
...
function _servicePath(csdl, csn, protocols) {
  if (csdl.$EntityContainer) {
    ...
    if (Array.isArray(protocols)) {
      protocols.forEach((protocol) => {
        const serviceWithProtocol = { ...service, "@protocol": protocol };
        path = cds.service.path4?.(serviceWithProtocol) || cds.serve.path4(serviceWithProtocol);
        paths[protocol] = path;
      });
    }

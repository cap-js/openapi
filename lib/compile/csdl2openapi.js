...
module.exports.csdl2openapi = function (
  ...
  if (csdl.$EntityContainer) {
    ...
    Object.keys(entityContainer).forEach((element) => {
      if (entityContainer[element].$Type) {
        ...
        if (
          (csdl[serviceName]?.[type]?.["@cds.autoexpose"] ||
            csdl[serviceName]?.[type]?.["@cds.autoexposed"]) &&
  ...
  const keyAsSegment = entityContainer
    ? entityContainer[voc.Capabilities.KeyAsSegmentSupported]
    : {};
  ...
  function checkForExtentionEnums(extensionObj, extensionEnums) {
    for (const [key, value] of Object.entries(extensionObj)) {
      if (
        ...
        !extensionEnums[key].allowedValues.includes(value)
      ) {
    ...
    for (const [key, value] of Object.entries(extensionObj)) {
      if (extenstionSchema[key]) {
        if (Array.isArray(value)) {
          extensionObj[key] = value.filter((v) =>
            extenstionSchema[key].includes(v),
          );
  ...
  function preProcess(
    ...
    Object.keys(csdl)
      ...
      .forEach((name) => {
        ...
        Object.keys(schema)
          .filter((iName) => isIdentifier(iName))
          .forEach((iName2) => {
              ...
              element
                ...
                .forEach((overload) => {
                  ...
                  boundOverloads[type].push({
                    name: isDefaultNamespace ? iName2 : qualifiedName,
                    overload: overload,
                  });
                });
  ...
  function getExternalDoc(csdl) {
    ...
    if (containerSchema?.["@OpenAPI.externalDocs.description"]) {
      externalDocs.description =
        containerSchema["@OpenAPI.externalDocs.description"];
    }
  ...
  function getTags(container) {
    ...
    Object.keys(container)
      ...
      .forEach((child) => {
        ...
        const description =
          container[child][voc.Core.Description] || type[voc.Core.Description];
        if (description) tag.description = description;
        tags.set(tag.name, tag);
      });
    return Array.from(tags.values()).sort((pre, next) =>
  ...
  function navigationPropertyRestrictions(root, navigationPath) {
    ...
    return (
      (navigationRestrictions.RestrictedProperties || []).find(
        (item) =>
          navigationPropertyPath(item.NavigationProperty) == navigationPath,
      ) || {}
    );
  }
  ...
  function operationCreate(
    ...
    const insertRestrictions =
      ...
      {};
    let countRestrictions =
      target && target[voc.Capabilities.CountRestrictions]?.Countable === false; // count property will be added if CountRestrictions is false
  ...
  function operationRead(
    ...
    let readable = true;
    let countRestrictions =
      target && target[voc.Capabilities.CountRestrictions]?.Countable === false;
    if (
      byKey &&
    ...
    if (readable) {
      ...
      const operation = {
        ...
        responses: response(
          200,
          "Retrieved " + (byKey ? pluralize.singular(lname) : lname),
...
see [Expand](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptionexpand)",
  ...
  function enumMember(member) {
    ...
    else if (typeof member == "object") return member.$EnumMember;
  }
  ...
  function navigationPropertyPath(path) {
    ...
    else return path.$NavigationPropertyPath;
  }
  ...
  function operationUpdate(
    ...
    if (updateRestrictions.Updatable !== false) {
      ...
      const updateMethod = updateRestrictions.UpdateMethod
        ? updateRestrictions.UpdateMethod.toLowerCase()
        : "patch";
  ...
  function pathItemsWithNavigation(
    ...
    const rootNavigable =
      (level == 0 &&
        enumMember(navigationRestrictions.Navigability) != "None") ||
      (level == 1 &&
        enumMember(navigationRestrictions.Navigability) != "Single") ||
      level > 1;
    ...
    if (type && level < maxLevels) {
      ...
      Object.keys(properties).forEach((name) => {
        ...
        if (
          ["Recursive", "Single"].includes(
            enumMember(restrictions.Navigability),
          ) ||
          ...
          const targetName =
            (targetType && targetType[voc.Common.Label]) || targetSetName;
          pathItems(
            paths,
            prefix + "/" + name,
            prefixParameters,
  ...
  function pathItemAction(
    ...
    const pathItem = {
      post: {
        summary:
          ...
          "Invokes action " + name,
        tags: [
          overload[voc.Common.Label] || sourceName || "Service Operations",
        ],
    ...
    if (parameters.length > 0) {
      ...
      parameters.forEach((p) => {
        requestProperties[p.$Name] = getSchema(p);
      });
  ...
  function pathItemBatch(paths, container) {
    ...
    if (supported) {
      ...
      paths["/$batch"] = {
        post: {
          summary:
            batchSupport[voc.Core.Description] || "Sends a group of requests",
          description:
    ...
    if (code != 204) {
      ...
      if (type.$Collection) {
        r[code].content["application/json"].schema = {
          ...
          title:
            "Collection of " +
            nameParts(type.$Type ? type.$Type : "Edm.String").name,
          properties: {
  ...
  function schemasForStructuredType(schemas, qualifier, name, type, suffix) {
    ...
    if (csdl[qualifier]?.$Annotations) {
      const annotations =
        csdl[qualifier].$Annotations[`${qualifier}.EntityContainer/${name}`];
      if (
        annotations &&
        annotations[voc.Capabilities.CountRestrictions] &&
  ...
  function odmExtensions(type, schema) {
    for (const [annotation, openApiExtension] of Object.entries(
      ODM_ANNOTATIONS,
    )) {
  ...
  function getSchema(
    ...
    switch (element.$Type) {
      ...
      case "Edm.Decimal":
        ...
        if (scale !== null) {
          ...
          if (scale <= 0) s.anyOf[0].multipleOf = 10 ** -scale;
          else s.anyOf[0].multipleOf = 1 / 10 ** scale;
        }
    ...
    if (element["@ODM.oidReference"]?.entityName) {
      s["x-sap-odm-oid-reference-entity-name"] =
        element["@ODM.oidReference"].entityName;
    }
    return s;
  }
  ...
  function getSecuritySchemes(components, entityContainer) {
    const authorizations =
      entityContainer && entityContainer[voc.Authorization.Authorizations]
  ...
  function security(openapi, entityContainer) {
    const securitySchemes =
      entityContainer && entityContainer[voc.Authorization.SecuritySchemes]

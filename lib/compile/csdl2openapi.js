    ...
    function preProcess(csdl, boundOverloads, derivedTypes, alias, namespace, namespaceUrl, voc) {
        ...
        Object.keys(csdl).filter(name => isIdentifier(name)).forEach(name => {
            ...
            Object.keys(schema.$Annotations || {}).forEach(target => {
                ...
                if (!element) {
                    console.debug(`Invalid annotation target '${target}'`);
                    ...
                    switch (segments.length) {
                        ...
                        case 2:
                            ...
                            break;
                        default:
                            console.debug('More than two annotation target path segments');
                    }
                }
            });
        });
    ...
    function getPaths(container) {
        ...
        resources.forEach(name => {
            ...
            if (child.$Type) {
                ...
                console.debug('Unrecognized entity container child: ' + name);
            }
        })
...
see [Expand](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptionexpand)',
    ...
    function primitivePaths(element, prefix = '') {
        ...
        if (!elementType) {
            console.debug(`Unknown type for element: ${JSON.stringify(element)}`);
            return paths;
        }
        ...
        // Keep old logging
        ignore.forEach(entry => console.debug(`Unknown type for element: ${JSON.stringify(entry)}`));
        ...
        for (let i = 0; i < properties.length; i++) {
            ...
            if (property.typeRefChain.filter(_type => _type === typeRefChainTail).length > 1) {
                console.debug(`Cycle detected ${property.typeRefChain.join('->')}`);
                continue;
            }
    ...
    */
        ...
        switch (element.$Type) {
            ...
            default:
                if (element.$Type.startsWith('Edm.')) {
                    console.debug('Unknown type: ' + element.$Type);
                } else {
    ...
    function getSecuritySchemes(components, entityContainer) {
        ...
        authorizations.forEach(auth => {
            ...
            switch (type) {
                ...
                default:
                    unknown = true
                    console.debug('Unknown Authorization type ' + qualifiedType);
            }
    ...
    function security(openapi, entityContainer) {
        ...
        if (securitySchemes.length === 0) {
            console.debug('No security schemes defined in the entity container');
        }

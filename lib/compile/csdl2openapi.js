...
/**
 * @typedef {{
 *  [K in keyof typeof terms]: {
 *   [P in typeof terms[K][number]]: string;
 *  }
 * } & { Common: { Label: string } }} Vocabulary
 */
...
const terms = {
    Authorization: /**@type{const}*/(['Authorizations', 'SecuritySchemes']),
    Capabilities: /**@type{const}*/([
        'BatchSupport', 'BatchSupported', 'ChangeTracking', 'CountRestrictions', 'DeleteRestrictions', 'DeepUpdateSupport', 'ExpandRestrictions',
    ...
    Core: /**@type{const}*/([
        'AcceptableMediaTypes', 'Computed', 'ComputedDefaultValue', 'DefaultNamespace', 'Description', 'Example', 'Immutable', 'LongDescription',
        'OptionalParameter', 'Permissions', 'SchemaVersion']),
    JSON: /**@type{const}*/(['Schema']),
    Validation: /**@type{const}*/(['AllowedValues', 'Exclusive', 'Maximum', 'Minimum', 'Pattern'])
};
...
module.exports.csdl2openapi = function (
    ...
    const namespaceUrl = {};
    /** @type {Vocabulary} */
    // @ts-expect-error type will be fixed by reference in preProcess
    const voc = {};
     ...
     * @param {Record<string, string>} namespace Map of namespace or alias to namespace
     * @param {object} namespaceUrl Map of namespace to reference URL
     * @param {Vocabulary} voc Map of vocabularies and terms
     */
    function preProcess(csdl, boundOverloads, derivedTypes, alias, namespace, namespaceUrl, voc) {
        ...
        Object.keys(csdl).filter(name => isIdentifier(name)).forEach(name => {
            ...
            Object.keys(schema.$Annotations || {}).forEach(target => {
                ...
                let element;
                if (open == -1) {
                    element = modelElement(firstSegment);
                } else {
                    element = modelElement(firstSegment.substring(0, open));
     ...
     * @return {import('./types').Paths} Paths Object
     */
     ...
     * @param {import('./types').Paths} paths Paths Object to augment
...
see [Expand](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptionexpand)',
    ...
    */
     ...
     * @returns {Record<string, import('./types').Response>}
     */

...
see [Expand](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptionexpand)',
    ...
    function entityKey(entityType, level) {
        ...
        keys.forEach((key, index) => {
            ...
            if (property[meta.voc.Core.LongDescription]) {
                param.description = property[meta.voc.Core.LongDescription];
                if (property[meta.voc.Core.Description]) {
                    param.summary = property[meta.voc.Core.Description];
                }
            } else if (property[meta.voc.Core.Description]) {
                param.description = property[meta.voc.Core.Description];
            } else {
                param.description = 'key: ' + parameter;
            }
            params.push(param);
        })
    ...
    */
    ...
    function pathItemFunction(paths, prefix, prefixParameters, functionName, overload, sourceName, functionImport = {}) {
        ...
        parameters.forEach(p => {
            ...
            if (p[meta.voc.Core.LongDescription]) {
                param.description = p[meta.voc.Core.LongDescription];
                if (p[meta.voc.Core.Description]) {
                    param.summary = p[meta.voc.Core.Description];
                }
            } else if (p[meta.voc.Core.Description]) {
                param.description = p[meta.voc.Core.Description];
            }
    ...
    function getSchema(element, suffix = '', forParameter = false, forFunction = false) {
        ...
        const description = !forParameter && (element[meta.voc.Core.LongDescription] || element[meta.voc.Core.Description]);
        if (description) {
            if (s.$ref) s = { allOf: [s] };

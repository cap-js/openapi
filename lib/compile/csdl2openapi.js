...
module.exports.csdl2openapi = function (
    ...
    function getPaths(container) {
        ...
        resources.forEach(name => {
            ...
            if (child.$Type) {
                ...
                pathItems({ paths, prefix: `/${name}`, prefixParameters: [], element: child, root: child, sourceName, targetName: sourceName, target: child, level: 0, navigationPath: '' });
    ...
    function pathItems({ paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level, navigationPath }) {
        ...
        operationRead({ pathItem, element, name, sourceName, targetName, target, level, restrictions, byKey: false, nonExpandable });
        ...
        if (element.$ContainsTarget) {
            if (element.$Collection) {
                ...
                pathItemsForBoundOperations({ paths, prefix, prefixParameters, element, sourceName });
                pathItemsWithNavigation({ paths, prefix, prefixParameters, type, root, sourceName, level, navigationPrefix: navigationPath });
            }
        }
    ...
    function pathItemsWithKey({ paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level, navigationPath, restrictions, nonExpandable }) {
        ...
        if (restrictions.IndexableByKey == true || restrictions.IndexableByKey != false && targetIndexable) {
            ...
            if (key.parameters.length > 0) {
                ...
                operationRead({ pathItem, element, name, sourceName, targetName, target, level, restrictions, byKey: true, nonExpandable });
                if (!root['$cds.autoexpose']) {
                    operationUpdate({ pathItem, element, name, sourceName, target, level, restrictions, byKey: true });
                    operationDelete({ pathItem, element, name, sourceName, target, level, restrictions, byKey: true });
                }

                pathItemsForBoundOperations({ paths, prefix: path, prefixParameters: parameters, element, sourceName, byKey: true });
                pathItemsWithNavigation({ paths, prefix: path, prefixParameters: parameters, type, root, sourceName, level, navigationPrefix: navigationPath });
     ...
     * @param {object} options
     * @param {object} options.pathItem Path Item Object to augment
     * @param {object} options.element Model element of navigation segment
    ...
    function operationCreate({ pathItem, element, name, sourceName, targetName, target, level, restrictions }) {
        ...
        if (insertRestrictions.Insertable !== false) {
            ...
            const type = meta.modelElement(element.$Type);
            pathItem.post = {
                summary: insertRestrictions.Description || operationSummary({ operation: 'Creates', name, sourceName, level, collection: true, byKey: true }),
                ...
                responses: response({ code: 201, description: `Created ${lname}`, type: { $Type: element.$Type }, errors: insertRestrictions.ErrorResponses, isCount: !countRestrictions }),
            };
    ...
    function operationRead({ pathItem, element, name, sourceName, targetName, target, level, restrictions, byKey, nonExpandable }) {
        ...
        if (readable) {
            ...
            const operation = {
                summary: descriptions.Description || operationSummary({ operation: 'Retrieves', name, sourceName, level, collection: element.$Collection, byKey }),
                tags: [normaliseTag(sourceName)],
                parameters: [],
                responses: response({ code: 200, description: `Retrieved ${byKey ? pluralize.singular(lname) : lname}`, type: { $Type: element.$Type, $Collection: collection },
                    errors: byKey ? readByKeyRestrictions?.ErrorResponses : readRestrictions?.ErrorResponses, isCount: !countRestrictions })
            };
...
see [Expand](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptionexpand)',
    ...
    function operationUpdate({ pathItem, element, name, sourceName, target, level, restrictions, byKey = false }) {
        ...
        if (updateRestrictions.Updatable !== false && !element[meta.voc.Core.Immutable]) {
            ...
            const type = meta.modelElement(element.$Type);
            const operation = {
                summary: updateRestrictions.Description || operationSummary({ operation: 'Changes', name, sourceName, level, collection: element.$Collection, byKey }),
                tags: [normaliseTag(sourceName)],
                requestBody: {
                ...
                responses: response({ code: 204, description: "Success", type: undefined, errors: updateRestrictions.ErrorResponses, isCount: !countRestrictions }),
            };
    ...
    function operationDelete({ pathItem, element, name, sourceName, target, level, restrictions, byKey = false }) {
        ...
        if (deleteRestrictions.Deletable !== false) {
            pathItem.delete = {
                summary: deleteRestrictions.Description || operationSummary({ operation: 'Deletes', name, sourceName, level, collection: element.$Collection, byKey }),
                tags: [normaliseTag(sourceName)],
                responses: response({ code: 204, description: "Success", type: undefined, errors: deleteRestrictions.ErrorResponses, isCount: !countRestrictions }),
            };
     ...
     * @param {string} options.prefix Prefix for path
     * @param {Array} options.prefixParameters Parameter Objects for prefix
     * @param {object} options.type Entity type object of navigation segment
     * @param {object} options.root Root element
     * @param {string} options.sourceName Name of path source
     * @param {number} options.level Number of navigation segments so far
        ...
        if (type && level < maxLevels) {
            ...
            Object.keys(properties).forEach(name => {
                ...
                if (['Recursive', 'Single'].includes(enumMember(restrictions.Navigability))
                    ...
                    pathItems({ paths, prefix: `${prefix}/${name}`, prefixParameters, element: properties[name], root, sourceName, targetName, target, level: level + 1, navigationPath });
                }
            });
        }
    }
     ...
     * @param {string} options.prefix Prefix for path
     * @param {Array} options.prefixParameters Parameter Objects for prefix
     * @param {object} options.element Model element the operations are bound to
     * @param {string} options.sourceName Name of path source
    ...
    function pathItemsForBoundOperations({ paths, prefix, prefixParameters, element, sourceName, byKey = false }) {
        ...
        overloads.forEach(item => {
            ...
            if (item.overload.$Kind == 'Action')
                pathItemAction({ paths, prefix: `${prefix}/${pathName}`, prefixParameters, actionName: item.name, overload: item.overload, sourceName });
            else
                pathItemFunction({ paths, prefix: `${prefix}/${pathName}`, prefixParameters, functionName: item.name, overload: item.overload, sourceName });
        });
    }
    ...
    */
    function pathItemActionImport(paths, name, child) {
        ...
        pathItemAction({ paths, prefix: `/${name}`, prefixParameters: [], actionName: child.$Action, overload, sourceName: child.$EntitySet, actionImport: child });
    }
     ...
     * @param {object} options.overload Function overload
     * @param {string} options.sourceName Name of path source
    ...
    function pathItemAction({ paths, prefix, prefixParameters, actionName, overload, sourceName, actionImport = {} }) {
        ...
        const pathItem = {
            post: {
                ...
                responses: overload.$ReturnType ? response({ code: 200, description: "Success", type: overload.$ReturnType, errors: overload[meta.voc.Capabilities.OperationRestrictions]?.ErrorResponses })
                    : response({ code: 204, description: "Success", type: undefined, errors: overload[meta.voc.Capabilities.OperationRestrictions]?.ErrorResponses }),
            }
        };
    ...
    function pathItemFunctionImport(paths, name, child) {
        ...
        overloads && overloads.filter(overload => !overload.$IsBound).forEach(overload => pathItemFunction({ paths, prefix: `/${name}`, prefixParameters: [], functionName: child.$Function, overload, sourceName: child.$EntitySet, functionImport: child }));
    }
    ...
    function pathItemFunction({ paths, prefix, prefixParameters, functionName, overload, sourceName, functionImport = {} }) {
        ...
        const pathItem = {
            get: {
                ...
                responses: response({ code: 200, description: "Success", type: overload.$ReturnType, errors: overload[meta.voc.Capabilities.OperationRestrictions]?.ErrorResponses }),
            }
        };
     ...
     * @param {object} options.type Response type object

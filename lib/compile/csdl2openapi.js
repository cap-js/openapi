...
// - CountRestrictions for GET collection-valued (containment) navigation
 ...
 * @param {{ url?: string, servers?: object, odataVersion?: string, scheme?: string,
 *          host?: string, basePath?: string, diagram?: boolean, maxLevels?: number }} options
...
module.exports.csdl2openapi = function (
    ...
    function getExtensions(csdl, level) {
        ...
        const extensionEnums = {
            "x-sap-compliance-level": {
                allowedValues: ["sap:base:v1", "sap:core:v1", "sap:core:v2"]
            },
            "x-sap-api-type": {
                allowedValues: ["ODATA", "ODATAV4", "REST", "SOAP"]
            },
            "x-sap-direction": {
                allowedValues: ["inbound", "outbound", "mixed"],
                default: "inbound"
            },
            "x-sap-dpp-entity-semantics": {
                allowedValues: ["sap:DataSubject", "sap:DataSubjectDetails", "sap:Other"]
            },
            "x-sap-dpp-field-semantics": {
                allowedValues: [
                    "sap:DataSubjectID", "sap:ConsentID", "sap:PurposeID",
                    "sap:ContractRelatedID", "sap:LegalEntityID", "sap:DataControllerID",
                    "sap:UserID", "sap:EndOfBusinessDate", "sap:BlockingDate",
    ...
    function getInfo(csdl, entityContainer) {
        ...
        if (entityContainer && entityContainer[meta.voc.Core.LongDescription]) {
            ...
            description = "Use @Core.LongDescription: '...' or @Core.Description: '...' " +
     ...
     */
    function pathItems(
        paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level,
        navigationPath
    ) {
        ...
        operationRead(
            pathItem, element, name, sourceName, targetName, target, level, restrictions, false,
            nonExpandable
        );
        if (!root['$cds.autoexpose'] && element.$Collection &&
    ...
    function navigationPropertyRestrictions(root, navigationPath) {
        ...
        return (navigationRestrictions.RestrictedProperties ?? []).find(
            item => navigationPropertyPath(item.NavigationProperty) == navigationPath
        ) ?? {};
    }
    ...
    function pathItemsWithKey(
        paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level,
        navigationPath, restrictions, nonExpandable
    ) {
    ...
    function operationCreate(
        ...
        const insertRestrictions = restrictions.InsertRestrictions ||
            target?.[meta.voc.Capabilities.InsertRestrictions] || {};
        const countRestrictions =
            target?.[meta.voc.Capabilities.CountRestrictions]?.Countable === false
        if (insertRestrictions.Insertable !== false) {
            const lname = pluralize.singular(splitName(name));
            const type = meta.modelElement(element.$Type);
            pathItem.post = {
                summary: insertRestrictions.Description ||
                    operationSummary('Creates', name, sourceName, level, true, true),
                ...
                responses: response(
                    201, `Created ${lname}`, { $Type: element.$Type },
                    insertRestrictions.ErrorResponses, !countRestrictions
                ),
            };
    ...
    function operationSummary(operation, name, sourceName, level, collection, byKey) {
        ...
        const prefix = byKey ? 'a single ' : (collection ? 'a list of ' : '');
        const itemName = byKey ? pluralize.singular(lname) : lname;
        //TODO: suppress "a" for all singletons
        const suffix = level == 0 ? '' :
            (level == 1 && sname == 'me' ? ' of me' : ` of a ${pluralize.singular(sname)}`);
        return `${operation} ${prefix}${itemName}${suffix}.`;
    }
    ...
    function operationRead(
        pathItem, element, name, sourceName, targetName, target, level, restrictions, byKey,
        nonExpandable
    ) {
        ...
        let readable = true;
        const countRestrictions = target &&
            (target[meta.voc.Capabilities.CountRestrictions]?.Countable === false);
        ...
        if (readable) {
            ...
            const operation = {
                summary: descriptions.Description ||
                    operationSummary('Retrieves', name, sourceName, level, element.$Collection, byKey),
                tags: [normaliseTag(sourceName)],
                parameters: [],
                responses: response(
                    200, `Retrieved ${byKey ? pluralize.singular(lname) : lname}`,
                    { $Type: element.$Type, $Collection: collection },
                    byKey ? readByKeyRestrictions?.ErrorResponses : readRestrictions?.ErrorResponses,
                    !countRestrictions
                )
            };
            const deltaSupported = element[meta.voc.Capabilities.ChangeTracking] &&
            ...
            if (!byKey && deltaSupported) {
                ...
                operation.responses[200].content['application/json'].schema.properties['@odata.deltaLink'] = {
                    type: 'string',
                    example: `${basePath}/${name}?$deltatoken=opaque ` +
                        'server-generated token for fetching the delta'
                }
            }
...
see [Expand](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptionexpand)',
    ...
    function optionFilter(parameters, target, restrictions) {
        const filterRestrictions = restrictions.FilterRestrictions ||
            target?.[meta.voc.Capabilities.FilterRestrictions] || {};
        ...
        if (filterRestrictions.Filterable !== false) {
            const filter = {
                ...
                description: filterRestrictions[meta.voc.Core.Description] ||
                    ...
                    'odata-v4.01-part1-protocol.html#sec_SystemQueryOptionfilter)',
                in: 'query',
                schema: {
                    type: 'string'
                }
    ...
    function optionSearch(parameters, target, restrictions) {
        const searchRestrictions = restrictions.SearchRestrictions ??
            target?.[meta.voc.Capabilities.SearchRestrictions] ?? {};
    ...
    function operationUpdate(
        pathItem, element, name, sourceName, target, level, restrictions, byKey = false
    ) {
        const updateRestrictions = restrictions.UpdateRestrictions ||
            target?.[meta.voc.Capabilities.UpdateRestrictions] || {};
        const countRestrictions =
            target?.[meta.voc.Capabilities.CountRestrictions]?.Countable === false;
        if (updateRestrictions.Updatable !== false && !element[meta.voc.Core.Immutable]) {
            ...
            const operation = {
                summary: updateRestrictions.Description ||
                    operationSummary('Changes', name, sourceName, level, element.$Collection, byKey),
                tags: [normaliseTag(sourceName)],
                requestBody: {
    ...
    function operationDelete(
        ...
        const deleteRestrictions = restrictions.DeleteRestrictions ||
            target?.[meta.voc.Capabilities.DeleteRestrictions] || {};
        const countRestrictions =
            target?.[meta.voc.Capabilities.CountRestrictions]?.Countable === false;
        if (deleteRestrictions.Deletable !== false) {
            pathItem.delete = {
                summary: deleteRestrictions.Description ||
                    operationSummary('Deletes', name, sourceName, level, element.$Collection, byKey),
                tags: [normaliseTag(sourceName)],
                responses: response(
                    204, "Success", undefined, deleteRestrictions.ErrorResponses, !countRestrictions
                ),
            };
    ...
    function pathItemsWithNavigation(
        paths, prefix, prefixParameters, type, root, sourceName, level, navigationPrefix
    ) {
        const navigationRestrictions = root[meta.voc.Capabilities.NavigationRestrictions] ?? {};
        ...
        if (type && level < maxLevels) {
            ...
            Object.keys(properties).forEach(name => {
                ...
                if (['Recursive', 'Single'].includes(enumMember(restrictions.Navigability))
                    ...
                    const targetName = (targetType && targetType[meta.voc.Common.Label])
                        || targetSetName;
    ...
    function pathValueSuffix(typename) {
        //TODO: handle other Edm types, enumeration types, and type definitions
        if (['Edm.Int64', 'Edm.Int32', 'Edm.Int16', 'Edm.SByte', 'Edm.Byte',
            'Edm.Double', 'Edm.Single', 'Edm.Date', 'Edm.DateTimeOffset', 'Edm.Guid'
    ...
    */
    ...
    function pathItemAction(
        paths, prefix, prefixParameters, actionName, overload, sourceName, actionImport = {}
    ) {
        const name = actionName.indexOf('.') === -1 ? actionName : nameParts(actionName).name;
        const pathItem = {
            post: {
                summary: actionImport[meta.voc.Core.Description] ||
                    overload[meta.voc.Core.Description] || `Invokes action ${name}`,
                tags: [normaliseTag(
                    overload[meta.voc.Common.Label] || sourceName || 'Service Operations'
                )],
                responses: overload.$ReturnType ?
                    response(
                        200, "Success", overload.$ReturnType,
                        overload[meta.voc.Capabilities.OperationRestrictions]?.ErrorResponses
                    ) :
                    response(
                        204, "Success", undefined,
                        overload[meta.voc.Capabilities.OperationRestrictions]?.ErrorResponses
                    ),
            }
        };
    ...
    function pathItemFunctionImport(paths, name, child) {
        ...
        overloads && overloads.filter(overload => !overload.$IsBound).forEach(overload =>
            pathItemFunction(paths, `/${name}`, [], child.$Function, overload, child.$EntitySet, child)
        );
    }
    ...
    function pathItemFunction(
        paths, prefix, prefixParameters, functionName, overload, sourceName, functionImport = {}
    ) {
        ...
        parameters.forEach(p => {
            ...
            if (p.$Collection || p.$Type == 'Edm.Stream'
                ...
                param.description += `This is ${p.$Collection ? 'a ' : ''
                }URL-encoded JSON ${
        ...
        const pathItem = {
            get: {
                summary: functionImport[meta.voc.Core.Description]
                    || overload[meta.voc.Core.Description] || `Invokes function ${name}`,
    ...
    function propertiesOfStructuredType(type) {
        const properties = (type && type.$BaseType)
            ? propertiesOfStructuredType(meta.modelElement(type.$BaseType))
            : {};
        if (type) {
    ...
    function getSchema(element, suffix = '', forParameter = false, forFunction = false) {
        ...
        switch (element.$Type) {
            ...
            case 'Edm.DateTimeOffset':
                s = {
                    type: 'string',
                    format: 'date-time',
                    example: `2017-04-13T15:51:04${isNaN(element.$Precision) || element.$Precision === 0
                        ? ''
                        : `.${'0'.repeat(element.$Precision)}`}Z`
                };
                break;
            case 'Edm.Decimal': {
        ...
        const description = forParameter
            ? undefined
            : (element[meta.voc.Core.LongDescription] || element[meta.voc.Core.Description]);
        if (description) {
            if (s.$ref) s = { allOf: [s] };
    ...
    function getSecuritySchemes(components, entityContainer) {
        const authorizations = entityContainer && entityContainer[meta.voc.Authorization.Authorizations]
            ? entityContainer[meta.voc.Authorization.Authorizations]
            : [];
        const schemes = {};
    ...
    function security(openapi, entityContainer) {
        const securitySchemes = entityContainer && entityContainer[meta.voc.Authorization.SecuritySchemes]
            ? entityContainer[meta.voc.Authorization.SecuritySchemes]
            : [];

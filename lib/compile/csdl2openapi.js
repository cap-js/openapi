...
see [Expand](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptionexpand)',
    ...
    function primitivePaths(element, prefix = '') {
        ...
        for (let i = 0; i < properties.length; i++) {
            const property = /** @type{NonNullable<typeof properties[number]>} */(properties[i]);
    ...
    function entityKey(entityType, level) {
        ...
        keys.forEach((key, index) => {
            ...
            if (typeof key == 'string') {
                ...
                // @ts-expect-error - this can be asserted to be attached once we stricten types for getKey, etc
    ...
    function getSchema(element, suffix = '', forParameter = false, forFunction = false) {
        ...
        switch (element.$Type) {
            ...
            case 'Edm.Decimal': {
                ...
                if (scale !== null) {
                    // Node.js 24 has problems with negative exponents, 10 ** -5 --> 0.000009999999999999999
                    if (scale <= 0)

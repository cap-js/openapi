...
module.exports.csdl2openapi = function (
    ...
    const extensionAnnotationObj = getExtensions(csdl);



    if (extensionAnnotationObj && Object.keys(extensionAnnotationObj).length > 0) {
        openapi['x-sap-annotations'] = extensionAnnotationObj;
    }
    ...
    function getExtensionsAsyncapi(csdl) {
        let extensionObj = {};
    ...
    // write a function to get the extensions for openapi lik getExtensionsOpenapi taking the reference of getExtensionsAsyncapi but instead of csn use csdl

    function getExtensions(csdl) {
        let extensionObj = {};
        for (const [key, value] of Object.entries(csdl)) {

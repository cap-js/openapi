...
see [Expand](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptionexpand)',
    ...
    function pathItemsForMediaStream({ paths, prefix, prefixParameters, type, name, sourceName }) {
        if (type.$HasStream) {
            ...
            const mediaContent = Object.fromEntries(contentTypes.map(ct => [ct, {}]));
            const lname = splitName(name);
        ...
        Object.keys(properties).forEach(propName => {
            ...
            const mediaType = prop[meta.voc.Core.MediaType];
            const contentTypes = mediaType ? [mediaType] : ['*/*'];
            const mediaContent = Object.fromEntries(contentTypes.map(ct => [ct, {}]));
            const lname = splitName(name);
            const propPath = `${prefix}/${propName}`;
            const pathItem = { parameters: prefixParameters };

...
const { preProcess } = require('./preprocessing');
// Import OpenAPI info and extensions utilities
const { getInfo, getExternalDoc, getShortText, getExtensions } = require('./openapi-info');
const { namespaceQualifiedName, enumMember, isIdentifier, nameParts, modelElement, propertiesOfStructuredType, propertyPath, navigationPropertyPath } = require('./model-navigation')
...
module.exports.csdl2openapi = function (
    ...
    const namespaceUrl = {};
    /** @type {import('./preprocessing').Vocabulary} */
    // @ts-expect-error - voc is modified by reference to match the type
    const voc = {};
    ...
    const entityContainer = csdl.$EntityContainer ? modelElement(csdl.$EntityContainer, csdl, namespace) : {};
    ...
    const openapi = {
        openapi: '3.0.2',
        info: getInfo(csdl, entityContainer, voc, diagram, namespace),
        'x-sap-api-type': 'ODATAV4',
        'x-odata-version': csdl.$Version,
        'x-sap-shortText': getShortText(csdl, entityContainer, voc),
    ...
    function getTags(container) {
        ...
        Object.keys(container)
            ...
            .forEach(child => {
                const type = modelElement(container[child].$Type, csdl, namespace) || {};
                const tag = {
    ...
    function getPaths(container) {
        ...
        resources.forEach(name => {
            ...
            if (child.$Type) {
                const type = modelElement(child.$Type, csdl, namespace);
     ...
     * @param {null | import('./types').TargetRestrictions[]} target Target container child of path
    ...
    function pathItems(paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level, navigationPath) {
        ...
        const type = modelElement(element.$Type, csdl, namespace);
        const pathItem = {};
    ...
    function operationCreate(pathItem, element, name, sourceName, targetName, target, level, restrictions) {
        const insertRestrictions = restrictions.InsertRestrictions || target?.[voc.Capabilities.InsertRestrictions] || {};
        let countRestrictions = target?.[voc.Capabilities.CountRestrictions]?.Countable === false // count property will be added if CountRestrictions is false
    ...
    function optionCount(parameters, target) {
        const targetRestrictions = target?.[voc.Capabilities.CountRestrictions];
    ...
    function optionExpand(parameters, element, target, nonExpandable) {
        const targetRestrictions = target?.[voc.Capabilities.ExpandRestrictions];
...
see [Expand](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptionexpand)',
    ...
    function navigationPaths(element, prefix = '', level = 0) {
        const paths = [];
        const type = modelElement(element.$Type, csdl, namespace);
        const properties = propertiesOfStructuredType(type, csdl, namespace);
    ...
    function optionFilter(parameters, target, restrictions) {
        const filterRestrictions = restrictions.FilterRestrictions || target?.[voc.Capabilities.FilterRestrictions] || {};
    ...
    function primitivePaths(element, prefix = '') {
        const paths = [];
        const elementType = modelElement(element.$Type, csdl, namespace);
        ...
        const propsOfType = propertiesOfStructuredType(elementType, csdl, namespace);
        const ignore = Object.entries(propsOfType)
            ...
            .filter(entry => !modelElement(entry[1].$Type, csdl, namespace));
    ...
    function entryToProperty(parent) {
        ...
        return function (entry) {
            ...
            const propertyType = property.$Type && modelElement(property.$Type, csdl, namespace);

            if (propertyType && propertyType.$Kind && propertyType.$Kind === 'ComplexType') {
                return {
                    properties: propertiesOfStructuredType(propertyType, csdl, namespace),
    ...
    function optionSearch(parameters, target, restrictions) {
        const searchRestrictions = restrictions.SearchRestrictions ?? target?.[voc.Capabilities.SearchRestrictions] ?? {};
    ...
    function optionSelect(parameters, element, target, restrictions) {
        const selectSupport = restrictions.SelectSupport ?? target?.[voc.Capabilities.SelectSupport] ?? {};
    ...
    function operationUpdate(pathItem, element, name, sourceName, target, level, restrictions, byKey = false) {
        const updateRestrictions = restrictions.UpdateRestrictions || target?.[voc.Capabilities.UpdateRestrictions] || {};
        let countRestrictions = target?.[voc.Capabilities.CountRestrictions]?.Countable === false;
    ...
    function operationDelete(pathItem, element, name, sourceName, target, level, restrictions, byKey = false) {
        const deleteRestrictions = restrictions.DeleteRestrictions || target?.[voc.Capabilities.DeleteRestrictions] || {};
        let countRestrictions = target?.[voc.Capabilities.CountRestrictions]?.Countable === false
        ...
        if (type && level < maxLevels) {
            ...
            Object.keys(properties).forEach(name => {
                ...
                if (['Recursive', 'Single'].includes(enumMember(restrictions.Navigability))
                    ...
                    const targetType = target && modelElement(target.$Type, csdl, namespace);
    ...
    function navigationPathMap(type, map = {}, prefix = '', level = 0) {
        ...
        Object.keys(properties).forEach(key => {
            if (properties[key].$Kind == 'NavigationProperty') {
                ...
                navigationPathMap(modelElement(properties[key].$Type, csdl, namespace), map, prefix + key + '/', level + 1);
            }
        })
        return map;
    }
    ...
    function getKey(entityType) {
        ...
        while (type) {
            ...
            type = modelElement(type.$BaseType, csdl, namespace);
        }
        return keys;
    }
    ...
    function entityKey(entityType, level) {
        ...
        const properties = propertiesOfStructuredType(entityType, csdl, namespace);
        ...
        keys.forEach((key, index) => {
            ...
            if (typeof key == 'string') {
                ...
                for (let i = 1; i < segments.length; i++) {
                    const complexType = modelElement(property.$Type, csdl, namespace);
                    const properties = propertiesOfStructuredType(complexType, csdl, namespace);
    ...
    */
    ...
    function pathItemFunctionImport(paths, name, child) {
        const overloads = modelElement(child.$Function, csdl, namespace);
    ...
    function pathItemFunction(paths, prefix, prefixParameters, functionName, overload, sourceName, functionImport = {}) {
        ...
        parameters.forEach(p => {
            ...
            const type = modelElement(p.$Type ?? 'Edm.String', csdl, namespace);
    ...
    function getSchemas(csdl) {
        ...
        for (const r of requiredSchemas.list) {
            const type = modelElement(`${r.namespace}.${r.name}`, csdl, namespace);
            if (!type) continue;
            switch (type.$Kind) {
                case "ComplexType":
                case "EntityType":
    ...
    function getSchema(element, suffix = '', forParameter = false, forFunction = false) {
        ...
        switch (element.$Type) {
            ...
            default:
                if (element.$Type.startsWith('Edm.')) {
                    ...
                    let type = modelElement(element.$Type, csdl, namespace);
        ...
        /** @returns {s is import('./types').StringSchema} */
        const isStringSchema = s => s?.type === 'string'
        /** @returns {s is import('./types').NumberSchema} */
        const isNumberSchema = s => s?.type === 'number' || s?.type === 'integer'
        /** @returns {s is import('./types').AnyOf} */
    ...
    function ref(typename, suffix = '') {
        ...
        if (typename.indexOf('.') != -1) {
            ...
            const nsp = namespace[parts.qualifier];
            name = nsp + '.' + parts.name;
    ...
    function security(openapi, entityContainer) {
        ...
        securitySchemes.forEach(scheme => {
            openapi.security.push({
                [scheme.Authorization]: scheme.RequiredScopes ?? []
            });
        });
    }
};

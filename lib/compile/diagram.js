...
class Diagram {
    ...
    getResourceDiagram(entityContainer) {
        ...
        Object.keys(this.#meta.csdl).filter(name => isIdentifier(name)).forEach(namespace => {
            ...
            Object.keys(schema)
                ...
                .forEach(typeName => {
                    ...
                    }[${typeName}${type.$Kind == 'EntityType' ?
                        this.#color.entityType : this.#color.complexType}]`;
                    Object.keys(type).filter(name => isIdentifier(name)).forEach(propertyName => {
                        ...
                        if (property.$Kind == 'NavigationProperty' || targetNP.qualifier != 'Edm') {
                            ...
                            const bidirectional = property.$Partner && target &&
                            ...
                            if (!bidirectional || propertyName <= property.$Partner) {
                                diagram += `,[${typeName}]${
                                    ...
                                    '++' : (bidirectional ? cardinality(target[property.$Partner]) : '')
        ...
        if (diagram != '') {
            diagram = `\n\n## Entity Data Model\n` +
                ...
                `![Legend](https://yuml.me/diagram/plain;dir:TB;scale:60/class/` +
                `[External.Type${this.#color.external}],` +
                `[ComplexType${this.#color.complexType}],` +
                `[EntityType${this.#color.entityType}],` +
    ...
    #overloadDiagram(name, overload) {
        ...
        for (const param of overload.$Parameter || []) {
            ...
            if (type) {
                diag += `${this.#comma}[${name}${this.#color.resource}]in-` +
                    `${cardinality(param.$Type)}>[${nameParts(param.$Type).name}]`;
            }
        }
        return diag;
    }

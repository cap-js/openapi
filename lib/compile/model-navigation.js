...
function propertiesOfStructuredType(type, csdl, namespace) {
    const properties = (type && type.$BaseType)
        ? propertiesOfStructuredType(modelElement(type.$BaseType, csdl, namespace), csdl, namespace)
        : {};
    if (type) {
...
function enumMember(member) {
    if (typeof member === 'string')
        return member;
    if (typeof member === 'object' && member.$EnumMember)
...
function navigationPropertyPath(path) {
    if (typeof path === 'string')
        return path;
    if (typeof path === 'object' && path.$NavigationPropertyPath)
...
function propertyPath(path) {
    if (typeof path === 'string')
        return path;
    if (typeof path === 'object' && path.$PropertyPath)

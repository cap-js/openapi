...
- Calling `cds compile -2 openapi` now emits events: `before:compile.to.openapi` with an object parameter, containing the source model and the compilation options. `after:compile.to.openapi` carries the same parameter with an additional `result` property, containing the output of the compilation process.

...
describe("Edge cases", () => {
  ...
  test("function with complex and optional collection parameter", () => {
    ...
    assert.deepStrictEqual(
      actual.paths[path].get.tags,
      expected.paths[path].get.tags,
      "function tags"
    );
  });
  ...
  test("entity set and singleton with non-existing type", () => {
    ...
    assert.deepStrictEqual(
      actual.paths["/things"].post,
      expected.paths["/things"].post,
      "POST set"
    );
    assert.deepStrictEqual(
...
see [Expand](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptionexpand)",
  ...
  test("Default Namespace", () => {
    const csdl = {
      ...
      this: {
        ...
        root: {
          ...
          $Key: ["key"],
          key: {},
          other: { $Kind: "NavigationProperty", $Type: "this.other" },
        },
        other: {
          "@Common.Label": "Second",
    ...
    assert.deepStrictEqual(
      actual.paths["/roots/func"].get.tags,
      ["First"],
      "operation tag"
    );
    assert.deepStrictEqual(
      actual.paths["/roots"].get.tags,
      ["First"],
      "operation tag"
    );
    assert.deepStrictEqual(
      actual.paths["/roots"].post.tags,
      ["First"],
      "operation tag"
    );
    assert.deepStrictEqual(
      actual.paths["/roots/{key}"].get.tags,
      ["First"],
      "operation tag"
    );
    assert.deepStrictEqual(
      actual.paths["/roots/{key}"].patch.tags,
      ["First"],
      "operation tag"
    );
    assert.deepStrictEqual(
      actual.paths["/roots/{key}"].delete.tags,
      ["First"],
      "operation tag"
    );
    assert.deepStrictEqual(
      actual.paths["/roots/{key}/act"].post.tags,
      ["First"],
      "operation tag"
    );
    assert.deepStrictEqual(
      actual.paths["/roots/{key}/func"].get.tags,
      ["First"],
      "operation tag"
    );
    assert.deepStrictEqual(
      actual.paths["/roots/{key}/other"].get.tags,
      ["First", "Second"],
      "operation tag"
    );
    assert.deepStrictEqual(
      actual.tags,
      [
        { name: "First", description: "The first thingy" },
        { name: "Second", description: "The second thingy" },
      ],
      "global tags"
    );
  });
